    ...
    for (let colIdx = 0; colIdx < headers.length; colIdx++) {
        ...
        while (searchPos < rowText.length) {
            ...
            if (!isComplexValue) {
                ...
                const charAfter = index + (searchPattern as string).length < rowText.length 
                    ? rowText[index + (searchPattern as string).length] 
                    : ' ';
                ...
                if (validBefore && validAfter) {
                    ...
                    matchedLengthInRow = (searchPattern as string).length;
                    break;
                }
                ...
                for (let k = searchPos; k < rowText.length; k++) {
                    if (rowText[k] === '[' || rowText[k] === '{') { index = k; break; }
                }
                if (index === -1) break;
                ...
                try {
                    ...
                    if (JSON.stringify(parsedCandidate) === JSON.stringify(value)) {
                        ...
                        // advance searchStartInRow later using candidate length
                        searchPos = endIdx + 1;
                        break;
                    } else {
        ...
        try {
            ...
            // (allow decorations that span multiple lines)
